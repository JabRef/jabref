...
- We fixed an issue where searching for special characters (e.g. `*`, `{`, `[`) would crash the preview with an uncaught regex exception. A malformed explicit regex search (`=~`) now shows the existing "Illegal search expression" warning instead of silently failing to highlight. [#16539](https://github.com/JabRef/jabref/issues/16539)
