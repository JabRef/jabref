...
- We fixed a latent crash where the default cleanup preset still referenced the removed `CLEAN_UP_ISSN` step (ISSN normalization now runs via the `NormalizeIssn` field formatter); stored preferences referencing removed cleanup steps are now ignored instead of failing to load. [#15948](https://github.com/JabRef/jabref/pull/15948)
