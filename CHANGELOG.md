...
- We grouped the `jabkit` consistency and integrity checks under a new `check` command (`jabkit check consistency`, `jabkit check integrity`), and `jabkit check integrity` now defaults to the `txt` output format.
