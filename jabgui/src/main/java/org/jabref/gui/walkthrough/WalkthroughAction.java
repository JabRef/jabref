...
public class WalkthroughAction extends SimpleCommand {
    private static final Map<Pair<String, WeakReference<Stage>>, Walkthrough> WALKTHROUGH_CACHE = new WeakHashMap<>();
    ...
    @Override
    public void execute() {
        Pair<String, WeakReference<Stage>> key = new Pair<>(name, new WeakReference<>(mainStage));
        Walkthrough walkthrough = switch (name) {
            case "customizeEntryTable" -> WALKTHROUGH_CACHE
                    .computeIfAbsent(key, _ -> createEntryTableWalkthrough(mainStage));
            case "mainFileDirectory" -> WALKTHROUGH_CACHE
    ...
    private static Walkthrough createEntryTableWalkthrough(Stage stage) {
        ...
        WalkthroughStep fileMenuStep = builder.createFileMenuStep();
        WalkthroughStep preferencesStep = builder.createPreferencesStep();
        WalkthroughStep entryTableStep = builder.createTabSelectionStep(
                "Entry table",
        ...
        WalkthroughStep customizeStep = PanelStep
        ...
        WalkthroughStep saveStep = builder.createSaveStep(
        ...
        return new Walkthrough(fileMenuStep, preferencesStep, entryTableStep, customizeStep, saveStep);
    }
    ...
    private static Walkthrough createMainFileDirectoryWalkthrough(Stage stage) {
        ...
        WalkthroughStep fileMenuStep = builder.createFileMenuStep();
        WalkthroughStep preferencesStep = builder.createPreferencesStep();
        WalkthroughStep linkedFilesStep = builder.createTabSelectionStep(
                "Linked files",
                Localization.lang("This section manages how JabRef handles your PDF files and other documents.")
        );

        WalkthroughStep mainFileDirectoryStep = TooltipStep
        ...
        return new Walkthrough(fileMenuStep, preferencesStep, linkedFilesStep, mainFileDirectoryStep, saveStep);
    }
