...
public class MetaData {
    ...
    /// Whether external changes of the library file are merged into the library without asking; absent if the
    /// library does not decide this itself (then the global preference applies)
    public Optional<Boolean> getSynchronizeWithFile() {
        return Optional.ofNullable(synchronizeWithFile);
    }

    public void setSynchronizeWithFile(boolean synchronizeWithFile) {
        if (Objects.equals(this.synchronizeWithFile, synchronizeWithFile)) {
            return;
        }
        this.synchronizeWithFile = synchronizeWithFile;
        postChange();
    }

    public void clearSynchronizeWithFile() {
        if (this.synchronizeWithFile == null) {
            return;
        }
        this.synchronizeWithFile = null;
        postChange();
    }
