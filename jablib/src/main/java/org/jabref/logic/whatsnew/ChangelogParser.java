...
public final class ChangelogParser {
    ...
    /// Every entry of `lines`, keyed by the index of its `- ` line, in the order of the file. An indented line
    /// right after an entry continues it and joins its text with a space; a blank line ends the entry.
    public static SequencedMap<Integer, ChangelogEntry> entries(List<String> lines) {
        ...
        for (int lineIndex = 0; lineIndex < lines.size(); lineIndex++) {
            ...
            if (line.startsWith(SECTION_PREFIX)) {
                ...
                entryLine = lineIndex;
                entries.put(entryLine, new ChangelogEntry(section, heading, line.substring(ENTRY_PREFIX.length()).strip()));
            } else if (entryLine >= 0 && !line.isBlank() && Character.isWhitespace(line.charAt(0))) {
                ChangelogEntry entry = entries.get(entryLine);
                entries.put(entryLine, new ChangelogEntry(section, heading, entry.text() + " " + line.strip()));
            } else {
                entryLine = -1;
            }
        }
