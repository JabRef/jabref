...
public interface MergeBookkeeper {
    /**
     * Record the GUI-produced merge result into Git history without touching file bytes.
     * Creates the right commit shape based on the merge graph:
     *  - BEHIND: fast-forward if content equals remote; otherwise create a new commit on top of `remote`.
     ...
     */
    FinalizeResult resultRecord(Path bibFilePath, PullComputation computation) throws IOException, GitAPIException, JabRefException;
}
