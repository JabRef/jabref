...
public interface IdFetcher<T extends Identifier> extends WebFetcher {
    ...
    /// Looks for an identifier based on the information stored in the given [BibEntry].
    ///
    /// @param entry the [BibEntry] for which an identifier should be found
    /// @return the identifier (if an ID was found, otherwise an empty [Optional])
