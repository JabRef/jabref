...
/// Extracts identifier field values (DOI, ISBN, ISSN, eprint) from a PDF file
/// by parsing its XMP metadata and text content.
///
/// @implNote The supported fields must be consistent with
/// {@link WebFetchers#getIdBasedFetcherForField(Field, ImportFormatPreferences)}.
public class PdfIdentifierExtractor {
    ...
    /// Parses the given PDF and returns a map of identifier field to value
    /// for every supported identifier found in the file.
    /// The first non-empty value found per field wins (XMP metadata takes priority over content).
    public Map<Field, String> extract(Path filePath) {
        return extractIdentifiers(parsePdfForEntries(filePath));
    }

    /// Extracts identifier field values from a list of already-parsed entries.
    /// Use this overload when the entries have already been obtained by other importers
