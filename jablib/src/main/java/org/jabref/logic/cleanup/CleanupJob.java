...
public interface CleanupJob {
    ...
    /// Cleans up the given entry, routing all [BibEntry] field mutations through the provided scheduler.
    ///
    /// The scheduler must execute mutations synchronously (blocking the caller until complete), e.g.
    /// `UiTaskExecutor::runAndWaitInJavaFXThread` when called from a background thread,
    /// or `Runnable::run` when already on the correct thread.
    ///
    /// The default implementation wraps the entire [#cleanup(BibEntry)] call in the scheduler, which
