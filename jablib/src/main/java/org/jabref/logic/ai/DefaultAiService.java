...
/// The default implementation for the AI functionality.
...
public class DefaultAiService implements AiService {
    ...
    public Optional<PlainCitationParser> getLlmPlainCitationParser(ImportFormatPreferences importFormatPreferences) {
        ChatModel chatModel = getCurrentChatModel();
        if (chatModel == null) {
            return Optional.empty();
        }
        return Optional.of(new LlmPlainCitationParser(
    ...
    public void clearEmbeddingsFor(List<LinkedFile> linkedFiles, BibDatabaseContext bibDatabaseContext, FilePreferences filePreferences) {
        embeddingsCleaner.clearEmbeddingsFor(linkedFiles, bibDatabaseContext, filePreferences);
    }
    ...
    public void generateEmbeddingsForGroup(BibDatabaseContext context, AbstractGroup group, List<LinkedFile> linkedFiles, FilePreferences filePreferences) {
        if (!aiPreferences.getAiFeaturesEnabled() || !aiPreferences.getAutoGenerateEmbeddings()) {
            return;
        }
        ingestionTaskAggregator
    ...
    public void generateSummariesForEntries(BibDatabaseContext context, List<BibEntry> entries, FilePreferences filePreferences) {
        ...
        entries.forEach(entry ->
                summarizationTaskAggregator.start(
                        new GenerateSummaryTaskRequest(
                                filePreferences,
                                getCurrentChatModel(),
                                getCurrentSummarizator(),
