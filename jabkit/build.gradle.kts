...
dependencies {
    ...
    implementation(
        "io.github.darvil82:terminal-text-formatter"
    )
...
application {
    mainClass.set(
        "org.jabref.toolkit.JabKitLauncher"
    )
    mainModule.set(
        "org.jabref.jabkit"
    )
...
javaModulePackaging {
    ...
    targetsWithOs(
        ...
        options.addAll(
            "--icon",
            "$projectDir/../jabgui/src/main/resources/icons/JabRef-linux-icon-64.png",
        )
        packageTypes =
            listOf(
                "app-image"
...
tasks.register<JavaExec>(
    "runJabKitPortableSmokeTest"
) {
    group =
        "test"
    description =
        "Runs JabKit from test resources dir"
    mainClass =
        "org.jabref.toolkit.JabKitLauncher"
    mainModule.set(
        "org.jabref.jabkit"
