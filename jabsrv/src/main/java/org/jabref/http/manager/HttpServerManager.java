...
public class HttpServerManager implements AutoCloseable {
    ...
    public synchronized void start(CliPreferences preferences, SrvStateManager srvStateManager, URI uri, RemoteMessageHandler remoteMessageHandler) {
        if (httpServerThread != null) {
            LOGGER.warn("HTTP server manager already started, cannot start again.");
            return;
        }

        httpServerThread = new HttpServerThread(preferences, srvStateManager, uri, remoteMessageHandler);
