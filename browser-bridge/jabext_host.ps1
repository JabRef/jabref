...
# Background runspace: blocking native-messaging read loop. Minimal surface — it
# only reads frames and delivers replies; import commands would dispatch here too.
$ReaderScript = {
    param($Sync)
    ...
    while ($true) {
        ...
        # else: existing JabRefHost.ps1 import dispatch would go here
    }
    $Sync.Done = $true
