...
class ConvertMarkingToGroupsTest {
    ...
    void performMigrationForSingleEntry() {
        ...
        GroupTreeNode rootExpected = GroupTreeNode.fromGroup(
            DefaultGroupsFactory.getAllEntriesGroup()
        );
        GroupTreeNode markings = rootExpected.addSubgroup(
            new ExplicitGroup("Markings", GroupHierarchyType.INCLUDING, ',')
        );
        markings.addSubgroup(
            new ExplicitGroup("Nicolas:6", GroupHierarchyType.INCLUDING, ',')
        );

        assertEquals(
            Optional.empty(),
            entry.getField(InternalField.MARKED_INTERNAL)
        );
        assertEquals(
            Optional.of(rootExpected),
