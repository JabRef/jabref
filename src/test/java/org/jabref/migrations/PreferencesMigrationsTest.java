...
class PreferencesMigrationsTest {
    ...
    void previewStyleReviewToComment() {
        ...
        String newPreviewStyle = "<font face=\"sans-serif\">__NEWLINE__"
                + "Customized preview style using reviews and comments:__NEWLINE__"
                + "\\begin{comment}<BR><BR><b>Comment: </b> \\format[Markdown,HTMLChars(keep)]{\\comment} \\end{comment}__NEWLINE__"
                + "\\begin{comment} Something: \\format[Markdown,HTMLChars(keep)]{\\comment} special \\end{comment}__NEWLINE__"
                + "</font>__NEWLINE__";
