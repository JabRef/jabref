...
public class FromAuxGroups extends JDialog {
    ...
    public FromAuxGroups(JabRefFrame frame, String title, boolean modal,
    ...
    public ExplicitGroup getGeneratedGroup() {
        List<BibEntry> entries = auxParser.parse().getGeneratedBibDatabase().getEntries();
        try {
            ExplicitGroup group = new ExplicitGroup("SomePaperNameGroup", GroupHierarchyType.INDEPENDENT, Globals.prefs);
            group.add(entries);
            return group;
