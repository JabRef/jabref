...
public class MultiMergeEntriesView extends BaseDialog<BibEntry> {
    ...
    public MultiMergeEntriesView(
        ...
        ButtonType mergeEntries = new ButtonType(
            Localization.lang("Merge entries"),
            ButtonBar.ButtonData.OK_DONE
        );
        this.getDialogPane()
            .getButtonTypes()
            .setAll(ButtonType.CANCEL, mergeEntries);
        ...
        viewModel
            ...
            .addListener(
                (ListChangeListener<
                        MultiMergeEntriesViewModel.EntrySource
                    >) c -> {
                    while (c.next()) {
                        if (c.wasAdded()) {
    ...
    public void initialize() {
        topScrollPane
            .hvalueProperty()
            .bindBidirectional(centerScrollPane.hvalueProperty());
        leftScrollPane
            .vvalueProperty()
            .bindBidirectional(centerScrollPane.vvalueProperty());
        rightScrollPane
            .vvalueProperty()
            .bindBidirectional(centerScrollPane.vvalueProperty());
                ...
                failedSuppliers.setText(
                    viewModel.failedSuppliersProperty().get().isEmpty()
                        ...
                        : Localization.lang(
                            ...
                            String.join(
                                ", ",
                                viewModel.failedSuppliersProperty()
                            )
                        )
                )
            );
    ...
    private void fillDiffModes() {
        diffMode.setItems(
            FXCollections.observableList(
                List.of(DiffMode.PLAIN, DiffMode.WORD, DiffMode.CHARACTER)
            )
        );
    ...
    private void addColumn(
        MultiMergeEntriesViewModel.EntrySource entrySourceColumn
    ) {
        // add header
        int columnIndex = supplierHeader.getChildren().size();
        ToggleButton header = generateEntryHeader(
            entrySourceColumn,
            columnIndex
        );
    ...
    private ToggleButton generateEntryHeader(
        MultiMergeEntriesViewModel.EntrySource column,
        int columnIndex
    ) {
     ...
     */
    private void writeBibEntryToColumn(
        MultiMergeEntriesViewModel.EntrySource entrySourceColumn,
        int columnIndex
    ) {
        for (Map.Entry<Field, String> entry : entrySourceColumn
            .entryProperty()
            .get()
            .getFieldsObservable()
            .entrySet()) {
     ...
     */
    private void setupSourceButtonAction(
            ...
            .addListener((observable, oldValue, newValue) -> {
                if (newValue) {
                    optionsGrid
                        ...
                        .forEach(hbox ->
                            ((HBox) hbox).getChildrenUnmodifiable()
                                .stream()
                                .filter(ToggleButton.class::isInstance)
                                .forEach(toggleButton ->
                                    ((ToggleButton) toggleButton).setSelected(
                                            true
                                        )
                                )
                        );
            ...
            DefaultTaskExecutor.runInJavaFXThread(() -> {
                ...
                prefHeightProperty()
                    .bind(
                        ((Region) fieldEditor
                                .getChildren()
                                .get(row.rowIndex)).heightProperty()
                    );
                ...
                // Text
                DiffHighlightingEllipsingTextFlow buttonText =
                    new DiffHighlightingEllipsingTextFlow(
                        content,
                        viewModel
                            .mergedEntryProperty()
                            .get()
                ...
                if (field.equals(StandardField.DOI)) {
                    Button doiButton =
                        IconTheme.JabRefIcons.LOOKUP_IDENTIFIER.asButton();
                    HBox.setHgrow(doiButton, Priority.NEVER);
                    doiButton
                        .prefHeightProperty()
                        .bind(cellButton.heightProperty());
                    ...
                    doiButton.setOnAction(event -> {
                        DoiFetcher doiFetcher = new DoiFetcher(
                            preferences.getImportFormatPreferences()
                        );
                        doiButton.setDisable(true);
                        addSource(
                            Localization.lang("From DOI"),
                            () -> {
                                try {
                                    return doiFetcher
                                        .performSearchById(content)
                                        .get();
                                } catch (
                                    FetcherException | NoSuchElementException e
                                ) {
                                    LOGGER.warn(
                                        "Failed to fetch BibEntry for DOI {}",
                                        content,
                                        e
                                    );
                                    return null;
    ...
    public void addSource(String title, BibEntry entry) {
        viewModel.addSource(
            new MultiMergeEntriesViewModel.EntrySource(title, entry)
        );
    }

    public void addSource(String title, Supplier<BibEntry> supplier) {
        viewModel.addSource(
            new MultiMergeEntriesViewModel.EntrySource(
                title,
                supplier,
                taskExecutor
            )
    ...
    private class FieldRow {
        ...
        public FieldRow(Field field, int rowIndex) {
            ...
            fieldEditorCell.addEventFilter(
                KeyEvent.KEY_PRESSED,
                event -> toggleGroup.selectToggle(null)
            );
                ...
                .addListener((obs, oldValue, newValue) -> {
                    if (newValue == null) {
                        ...
                        viewModel
                            ...
                            .setField(
                                field,
                                ((DiffHighlightingEllipsingTextFlow) ((ToggleButton) newValue).getGraphic()).getFullText()
                            );
        ...
        private void addRow(Field field) {
            ...
            fieldHeaderLabel
                .prefHeightProperty()
                .bind(fieldEditorCell.heightProperty());
