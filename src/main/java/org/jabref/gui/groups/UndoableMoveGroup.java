...
class UndoableMoveGroup extends AbstractUndoableJabRefEdit {
    ...
    public UndoableMoveGroup(
        GroupTreeNodeViewModel root,
        MoveGroupChange moveChange
    ) {
    ...
    public void undo() {
        ...
        // TODO: NULL
        node.moveTo(
            root.getNode().getDescendant(pathToOldParent).get(),
            oldChildIndex
        );
    }
    ...
    public void redo() {
        ...
        GroupTreeNode oldParent = root
            .getNode()
            .getDescendant(pathToOldParent)
            .get(); // TODO: NULL
        GroupTreeNode node = oldParent.getChildAt(oldChildIndex).get(); // TODO:Null
        // TODO: NULL
        node.moveTo(
            root.getNode().getDescendant(pathToNewParent).get(),
            newChildIndex
        );
    }
}
