...
public class GroupTreeView extends BorderPane {
    ...
    private static final PseudoClass PSEUDOCLASS_ROOTELEMENT =
        PseudoClass.getPseudoClass("root");
    ...
    public GroupTreeView(
        ...
        this.getStylesheets()
            .add(
                Objects
                    .requireNonNull(
                        GroupTreeView.class.getResource("GroupTree.css")
                    )
                    .toExternalForm()
            );
        initialize();
    ...
    private void createNodes() {
        ...
        groupTree.setColumnResizePolicy(
            TreeTableView.CONSTRAINED_RESIZE_POLICY_FLEX_LAST_COLUMN
        );
        groupTree
            .getColumns()
            .addAll(List.of(mainColumn, numberColumn, expansionNodeColumn));
    ...
    private void initialize() {
        ...
        final Timer searchTask = FxTimer.create(
    ...
    private StackPane createNumberCell(GroupNodeViewModel group) {
        ...
        if (!group.isRoot()) {
            BindingsHelper.includePseudoClassWhen(
                node,
                PSEUDOCLASS_ANYSELECTED,
                group.anySelectedEntriesMatchedProperty()
            );
            BindingsHelper.includePseudoClassWhen(
                node,
                PSEUDOCLASS_ALLSELECTED,
            ...
            .bind(
                Bindings.createStringBinding(
                    () -> {
                        ...
                        double font_size = preferencesService
    ...
    private void handleOnDragDetected(
        ...
        for (TreeItem<GroupNodeViewModel> selectedItem : row
            .getTreeTableView()
            .getSelectionModel()
            .getSelectedItems()) {
    ...
    private void updateSelection(
        List<TreeItem<GroupNodeViewModel>> newSelectedGroups
    ) {
        if ((newSelectedGroups == null) || newSelectedGroups.isEmpty()) {
            viewModel.selectedGroupsProperty().clear();
        } else {
            List<GroupNodeViewModel> list = newSelectedGroups
                .stream()
                .filter(model ->
                    (model != null) &&
                    !(model.getValue().getGroupNode().getGroup() instanceof
                        AllEntriesGroup)
                )
    ...
    private Optional<TreeItem<GroupNodeViewModel>> getTreeItemByValue(
        TreeItem<GroupNodeViewModel> root,
        GroupNodeViewModel value
    ) {
        if (root == null) {
    ...
    private void initScrolling() {
        ...
        double heightOfOneNode = groupTree
        ...
        scrollableAreaHeight =
            Math.min(heightOfOneNode * 3.0, groupTree.getHeight() / 3.0);
    ...
    private class ContextAction extends SimpleCommand {
        ...
        public void execute() {
            switch (command) {
                ...
                case GROUP_REMOVE_KEEP_SUBGROUPS -> viewModel.removeGroupKeepSubgroups(
                    group
                );
                case GROUP_REMOVE_WITH_SUBGROUPS -> viewModel.removeGroupAndSubgroups(
                    group
                );
                case GROUP_EDIT -> {
                ...
                case GROUP_SUBGROUP_ADD -> viewModel.addNewSubgroup(
                    group,
                    GroupDialogHeader.SUBGROUP
                );
                case GROUP_SUBGROUP_REMOVE -> viewModel.removeSubgroups(group);
                case GROUP_SUBGROUP_SORT -> viewModel.sortAlphabeticallyRecursive(
                    group.getGroupNode()
                );
                case GROUP_SUBGROUP_SORT_REVERSE -> viewModel.sortReverseAlphabeticallyRecursive(
                    group.getGroupNode()
                );
                case GROUP_SUBGROUP_SORT_ENTRIES -> viewModel.sortEntriesRecursive(
                    group.getGroupNode()
                );
                case GROUP_SUBGROUP_SORT_ENTRIES_REVERSE -> viewModel.sortReverseEntriesRecursive(
                    group.getGroupNode()
                );
                case GROUP_ENTRIES_ADD -> viewModel.addSelectedEntries(group);
                case GROUP_ENTRIES_REMOVE -> viewModel.removeSelectedEntries(
                    group
                );
            }
        }
