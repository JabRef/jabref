...
** Separation of createReferenceMarkForCitationGroup and fillCitationMarkInCursor
   ...
   - Problems
     ...
     Possible solutions:
   - For (1):
     - (currently) recreate the mark instead of filling
     - Ensure, that we bracket the content, and fill within that.
       - We could probably use a zero-width space
         e.g. \u200b (or any character formatted as "hidden")
     - Do we have to bracket, or a single character is enough?
