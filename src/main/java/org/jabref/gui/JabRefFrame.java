...
public class JabRefFrame extends JFrame implements OutputPrinter {
	...
	private final AbstractAction jabrefWebPageAction = new OpenBrowserAction("https://jabref.org",
			Localization.menuTitle("Website"), Localization.lang("Opens JabRef's website"), IconTheme.getImage("about"),
	...
	private final AbstractAction jabrefFacebookAction = new OpenBrowserAction("https://www.facebook.com/JabRef/",
			"Facebook", Localization.lang("Opens JabRef's Facebook page"), IconTheme.JabRefIcon.FACEBOOK.getSmallIcon(),
			IconTheme.JabRefIcon.FACEBOOK.getIcon());
	private final AbstractAction jabrefTwitterAction = new OpenBrowserAction("https://twitter.com/jabref_org",
			"Twitter", Localization.lang("Opens JabRef's Twitter page"), IconTheme.JabRefIcon.TWITTER.getSmallIcon(),
			IconTheme.JabRefIcon.TWITTER.getIcon());
	...
	private final AbstractAction forkMeOnGitHubAction = new OpenBrowserAction("https://github.com/JabRef/jabref",
			Localization.menuTitle("Fork me on GitHub"), Localization.lang("Opens JabRef's GitHub page"),
			IconTheme.JabRefIcon.GITHUB.getSmallIcon(), IconTheme.JabRefIcon.GITHUB.getIcon());
	private final AbstractAction donationAction = new OpenBrowserAction("https://donations.jabref.org",
			Localization.menuTitle("Donate to JabRef"), Localization.lang("Donate to JabRef"),
			IconTheme.JabRefIcon.DONATE.getSmallIcon(), IconTheme.JabRefIcon.DONATE.getIcon());
	private final AbstractAction openForumAction = new OpenBrowserAction("http://discourse.jabref.org/",
			Localization.menuTitle("Online help forum"), Localization.lang("Online help forum"),
			IconTheme.JabRefIcon.FORUM.getSmallIcon(), IconTheme.JabRefIcon.FORUM.getIcon());
	private final AbstractAction help = new HelpAction(Localization.menuTitle("Online help"),
			Localization.lang("Online help"), HelpFile.CONTENTS, Globals.getKeyPrefs().getKey(KeyBinding.HELP));
	private final AbstractAction about = new AboutAction(Localization.menuTitle("About JabRef"),
	...
	private final AbstractAction editEntry = new GeneralAction(Actions.EDIT, Localization.menuTitle("Edit entry"),
			Localization.lang("Edit entry"), Globals.getKeyPrefs().getKey(KeyBinding.EDIT_ENTRY),
			IconTheme.JabRefIcon.EDIT_ENTRY.getIcon());
	private final AbstractAction focusTable = new GeneralAction(Actions.FOCUS_TABLE,
			...
			Globals.getKeyPrefs().getKey(KeyBinding.FOCUS_ENTRY_TABLE));
	private final AbstractAction save = new GeneralAction(Actions.SAVE, Localization.menuTitle("Save library"),
			Localization.lang("Save library"), Globals.getKeyPrefs().getKey(KeyBinding.SAVE_DATABASE),
	...
	private final AbstractAction undo = new GeneralAction(Actions.UNDO, Localization.menuTitle("Undo"),
			Localization.lang("Undo"), Globals.getKeyPrefs().getKey(KeyBinding.UNDO),
			IconTheme.JabRefIcon.UNDO.getIcon());
	private final AbstractAction redo = new GeneralAction(Actions.REDO, Localization.menuTitle("Redo"),
			Localization.lang("Redo"), Globals.getKeyPrefs().getKey(KeyBinding.REDO),
			IconTheme.JabRefIcon.REDO.getIcon());
	private final AbstractAction forward = new GeneralAction(Actions.FORWARD, Localization.menuTitle("Forward"),
			Localization.lang("Forward"), Globals.getKeyPrefs().getKey(KeyBinding.FORWARD),
			IconTheme.JabRefIcon.RIGHT.getIcon());
	private final AbstractAction back = new GeneralAction(Actions.BACK, Localization.menuTitle("Back"),
			Localization.lang("Back"), Globals.getKeyPrefs().getKey(KeyBinding.BACK),
			IconTheme.JabRefIcon.LEFT.getIcon());
	private final AbstractAction deleteEntry = new GeneralAction(Actions.DELETE, Localization.menuTitle("Delete entry"),
			Localization.lang("Delete entry"), Globals.getKeyPrefs().getKey(KeyBinding.DELETE_ENTRY),
			IconTheme.JabRefIcon.DELETE_ENTRY.getIcon());
	private final AbstractAction copy = new EditAction(Actions.COPY, Localization.menuTitle("Copy"),
			Localization.lang("Copy"), Globals.getKeyPrefs().getKey(KeyBinding.COPY),
	...
	private final AbstractAction paste = new EditAction(Actions.PASTE, Localization.menuTitle("Paste"),
			Localization.lang("Paste"), Globals.getKeyPrefs().getKey(KeyBinding.PASTE),
	...
	private final AbstractAction openConsole = new GeneralAction(Actions.OPEN_CONSOLE,
			Localization.menuTitle("Open terminal here"), Localization.lang("Open terminal here"),
			Globals.getKeyPrefs().getKey(KeyBinding.OPEN_CONSOLE), IconTheme.JabRefIcon.CONSOLE.getIcon());
	private final AbstractAction pullChangesFromSharedDatabase = new GeneralAction(
			Actions.PULL_CHANGES_FROM_SHARED_DATABASE, Localization.menuTitle("Pull changes from shared database"),
	...
	private final AbstractAction mark = new GeneralAction(Actions.MARK_ENTRIES, Localization.menuTitle("Mark entries"),
			Localization.lang("Mark entries"), Globals.getKeyPrefs().getKey(KeyBinding.MARK_ENTRIES),
			IconTheme.JabRefIcon.MARK_ENTRIES.getIcon());
	...
	private final AbstractAction unmarkAll = new GeneralAction(Actions.UNMARK_ALL,
			Localization.menuTitle("Unmark all"));
	...
	private final AbstractAction normalSearch = new GeneralAction(Actions.SEARCH, Localization.menuTitle("Search"),
			Localization.lang("Search"), Globals.getKeyPrefs().getKey(KeyBinding.SEARCH),
	...
	private final AbstractAction copyPreview = new GeneralAction(Actions.COPY_CITATION_HTML,
			Localization.lang("Copy preview"), Globals.getKeyPrefs().getKey(KeyBinding.COPY_PREVIEW));
	...
	private final AbstractAction copyKey = new GeneralAction(Actions.COPY_KEY,
			Localization.menuTitle("Copy BibTeX key"), Globals.getKeyPrefs().getKey(KeyBinding.COPY_BIBTEX_KEY));
	private final AbstractAction copyCiteKey = new GeneralAction(Actions.COPY_CITE_KEY,
			Localization.menuTitle("Copy \\cite{BibTeX key}"),
	...
	private final AbstractAction replaceAll = new GeneralAction(Actions.REPLACE_ALL,
			Localization.menuTitle("Replace string") + ELLIPSES,
			Globals.getKeyPrefs().getKey(KeyBinding.REPLACE_STRING));
	private final AbstractAction editPreamble = new GeneralAction(Actions.EDIT_PREAMBLE,
			Localization.menuTitle("Edit preamble"), Localization.lang("Edit preamble"));
	private final AbstractAction editStrings = new GeneralAction(Actions.EDIT_STRINGS,
			Localization.menuTitle("Edit strings"), Localization.lang("Edit strings"),
			Globals.getKeyPrefs().getKey(KeyBinding.EDIT_STRINGS), IconTheme.JabRefIcon.EDIT_STRINGS.getIcon());
	...
	private final AbstractAction addToGroup = new GeneralAction(Actions.ADD_TO_GROUP,
			Localization.lang("Add to group") + ELLIPSES);
	...
	private final AbstractAction moveToGroup = new GeneralAction(Actions.MOVE_TO_GROUP,
			Localization.lang("Move to group") + ELLIPSES);
	private final Action togglePreview = enableToggle(new GeneralAction(Actions.TOGGLE_PREVIEW,
			Localization.menuTitle("Toggle entry preview"), Localization.lang("Toggle entry preview"),
	...
	private final AbstractAction makeKeyAction = new GeneralAction(Actions.MAKE_KEY,
			Localization.menuTitle("Autogenerate BibTeX keys"), Localization.lang("Autogenerate BibTeX keys"),
			Globals.getKeyPrefs().getKey(KeyBinding.AUTOGENERATE_BIBTEX_KEYS), IconTheme.JabRefIcon.MAKE_KEY.getIcon());
	...
	private final AbstractAction openFile = new GeneralAction(Actions.OPEN_EXTERNAL_FILE,
			Localization.menuTitle("Open file"), Localization.lang("Open file"),
			Globals.getKeyPrefs().getKey(KeyBinding.OPEN_FILE), IconTheme.JabRefIcon.FILE.getIcon());
	private final AbstractAction openUrl = new GeneralAction(Actions.OPEN_URL,
			Localization.menuTitle("Open URL or DOI"), Localization.lang("Open URL or DOI"),
			Globals.getKeyPrefs().getKey(KeyBinding.OPEN_URL_OR_DOI), IconTheme.JabRefIcon.WWW.getIcon());
	...
	private final AbstractAction cleanupEntries = new GeneralAction(Actions.CLEANUP,
			Localization.menuTitle("Cleanup entries") + ELLIPSES, Localization.lang("Cleanup entries"),
			Globals.getKeyPrefs().getKey(KeyBinding.CLEANUP), IconTheme.JabRefIcon.CLEANUP_ENTRIES.getIcon());
	private final AbstractAction mergeEntries = new GeneralAction(Actions.MERGE_ENTRIES,
			Localization.menuTitle("Merge entries") + ELLIPSES, Localization.lang("Merge entries"),
	...
	private final JMenu lookupIdentifiers = JabRefFrame
			.subMenu(Localization.menuTitle("Look up document identifier..."));
	private final GeneralAction findUnlinkedFiles = new GeneralAction(FindUnlinkedFilesDialog.ACTION_COMMAND,
	...
	private final NewEntryAction newEntryAction = new NewEntryAction(this,
			Globals.getKeyPrefs().getKey(KeyBinding.NEW_ENTRY));
	...
	private static Action enableToggle(Action a, boolean initialValue) {
		// toggle only works correctly when the SELECTED_KEY is set to false or true
	...
	/**
	 * Takes a list of Object and calls the method setEnabled on them, depending on
	 * whether it is an Action or a Component.
	 *
	 * @param list
	...
	private void init() {
		...
		WindowLocation pw = new WindowLocation(this, JabRefPreferences.POS_X, JabRefPreferences.POS_Y,
		...
		/*
		 * The following state listener makes sure focus is registered with the correct
		 * database when the user switches tabs. Without this, cut/paste/copy operations
		 * would some times occur in the wrong tab.
		 */
		tabbedPane.addChangeListener(e -> {
			...
			Platform.runLater(() -> Globals.stateManager.activeDatabaseProperty()
					.setValue(Optional.of(currentBasePanel.getBibDatabaseContext())));
			...
			openOfficePanel.getToggleAction()
					.setSelected(sidePaneManager.isComponentVisible(OpenOfficeSidePanel.class));
		...
		// Note: The registration of Apple event is at the end of initialization,
		// because
		// if the events happen too early (ie when the window is not initialized yet),
		// the
	...
	private Void showTrackingNotification() {
		if (!Globals.prefs.shouldCollectTelemetry()) {
			...
			boolean shouldCollect = dialogService.showConfirmationDialogAndWait(
					...
					Localization.lang("Share anonymous statistics"), Localization.lang("Don't share"));
	...
	/**
	 * The MacAdapter calls this method when a "BIB" file has been double-clicked
	 * from the Finder.
	 */
	...
	public void about() {
		...
		// null as parameter is OK as the code of actionPerformed does not rely on the
		// data sent in the event.
		about.actionPerformed(null);
	}

	// General preferences dialog. The MacAdapter calls this method when
	// "Preferences..."
	// is selected from the application menu.
	 ...
	 * <p>
	 * FIXME: Currently some threads remain and therefore hinder JabRef to be closed
	 * properly
	 *
	 * @param filenames
	 *            the filenames of all currently opened files - used for storing
	 *            them if prefs openLastEdited is set to true
	 */
	...
	/**
	 * General info dialog. The MacAdapter calls this method when "Quit" is selected
	 * from the application menu, Cmd-Q is pressed, or "Quit" is selected from the
	 * Dock. The function returns a boolean indicating if quitting is ok or not.
	 * <p>
	...
	private void createToolBar() {
		...
		if (Globals.prefs.getBoolean(JabRefPreferences.SPECIALFIELDSENABLED)) {
			if (Globals.prefs.getBoolean(JabRefPreferences.SHOWCOLUMN_RANKING)) {
				JButton button = SpecialFieldDropDown.generateSpecialFieldButtonWithDropDown(SpecialField.RANKING,
						this);
				tlb.add(button);
			...
			if (Globals.prefs.getBoolean(JabRefPreferences.SHOWCOLUMN_PRIORITY)) {
				JButton button = SpecialFieldDropDown.generateSpecialFieldButtonWithDropDown(SpecialField.PRIORITY,
						this);
				tlb.add(button);
				specialFieldButtons.add(button);
			}
			...
			if (Globals.prefs.getBoolean(JabRefPreferences.SHOWCOLUMN_READ)) {
				JButton button = SpecialFieldDropDown.generateSpecialFieldButtonWithDropDown(SpecialField.READ_STATUS,
						this);
				tlb.add(button);
				specialFieldButtons.add(button);
			}
	...
	/**
	 * displays the String on the Status Line visible on the bottom of the JabRef
	 * mainframe
	 */
	...
	private void initActions() {
		...
		openDatabaseOnlyActions.addAll(Arrays.asList(manageSelectors, mergeDatabaseAction, newSubDatabaseAction, save,
				copyPreview, saveAs, saveSelectedAs, saveSelectedAsPlain, editModeAction, undo, redo, cut, deleteEntry,
				copy, paste, mark, markSpecific, unmark, unmarkAll, rankSubMenu, editEntry, selectAll, copyKey,
				copyCiteKey, copyKeyAndTitle, copyKeyAndLink, editPreamble, editStrings,
				groupSidePane.getToggleAction(), makeKeyAction, normalSearch, generalFetcher.getToggleAction(),
				mergeEntries, cleanupEntries, exportToClipboard, replaceAll, sendAsEmail, downloadFullText,
				lookupIdentifiers, writeXmpAction, openOfficePanel.getToggleAction(), findUnlinkedFiles, addToGroup,
				removeFromGroup, moveToGroup, autoLinkFile, resolveDuplicateKeys, openUrl, openFolder, openFile,
				togglePreview, dupliCheck, autoSetFile, newEntryAction, newSpec, customizeAction, plainTextImport,
				getMassSetField(), getManageKeywords(), pushExternalButton.getMenuAction(), closeDatabaseAction,
				getNextPreviewStyleAction(), getPreviousPreviewStyleAction(), checkIntegrity, databaseProperties,
				abbreviateIso, abbreviateMedline, unabbreviate, exportAll, exportSelected, importCurrent, saveAll,
				focusTable, increaseFontSize, decreseFontSize, defaultFontSize, toggleRelevance, toggleQualityAssured,
				togglePrinted, pushExternalButton.getComponent()));
	...
	private void trackOpenNewDatabase(BasePanel basePanel) {
		...
		Globals.getTelemetryClient()
				.ifPresent(client -> client.trackEvent("OpenNewDatabase", properties, measurements));
	}
	...
	/**
	 * Creates icons for the disabled state for all JMenuItems with FontBasedIcons
	 * in the given menuElement. This is necessary as Swing is not able to generate
	 * default disabled icons for font based icons.
	 *
	 * @param menuElement
	 *            the menuElement for which disabled icons should be generated
	 */
	...
	/**
	 * This method does the job of adding imported entries into the active database,
	 * or into a new one. It shows the ImportInspectionDialog if preferences
	 * indicate it should be used. Otherwise it imports directly.
	 *
	 * @param panel
	...
	/**
	 * Set the visibility of the progress bar in the right end of the status line at
	 ...
	 *            A selected entries list of the current base pane
	 * @return true, if the selected entry contains file. false, if multiple entries
	 *         are selected or the selected entry doesn't contains file
	 */
	 ...
	 * @param selectEntryList
	 *            A selected entries list of the current base pane
	 * @return true, if the selected entry contains url or doi. false, if multiple
	 *         entries are selected or the selected entry doesn't contains url or
	 *         doi
	 */
	private boolean isExistURLorDOI(List<BibEntry> selectEntryList) {
		if (selectEntryList.size() == 1) {
			BibEntry selectedEntry = selectEntryList.get(0);
			return (selectedEntry.getField(FieldName.URL).isPresent()
					|| selectedEntry.getField(FieldName.DOI).isPresent());
		}
		return false;
	}
	...
	private int showSaveDialog(String filename) {
		...
		return JOptionPane.showOptionDialog(JabRefFrame.this, Localization.lang("Library '%0' has changed.", filename),
				Localization.lang("Save before closing"), JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE,
	...
	private static class MyGlassPane extends JPanel {
		public MyGlassPane() {
			 ...
			 * 
			 * setLayout(new BorderLayout()); add(infoLabel, BorderLayout.CENTER);
			 */
	...
	private class ChangeTabAction extends MnemonicAwareAction {
		...
		public ChangeTabAction(boolean next) {
			...
			this.next = next;
			putValue(Action.ACCELERATOR_KEY, next ? Globals.getKeyPrefs().getKey(KeyBinding.NEXT_TAB)
					: Globals.getKeyPrefs().getKey(KeyBinding.PREVIOUS_TAB));
		}
	 ...
	 * component is kept track of by Globals.focusListener, and we call the action
	 * stored under the relevant name in its action map.
	 */
	...
	private class BibtexKeyPatternAction extends MnemonicAwareAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (bibtexKeyPatternDialog == null) {
				...
				// BibtexKeyPatternDialog allows for updating content based on currently
				// selected panel
	...
	private class ToolBar extends OSXCompatibleToolbar {
		...
		public void addAction(Action a) {
			...
			// create a disabled Icon for FontBasedIcons as Swing does not automatically
			// create one
