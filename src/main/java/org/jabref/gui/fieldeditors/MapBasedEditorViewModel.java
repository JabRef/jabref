...
public abstract class MapBasedEditorViewModel<T> extends OptionEditorViewModel<T> {
    ...
    protected T getValueFromString(String string) {
        try {
            ...
            LOGGER.error(String.format("Could not cast string to type %s. Try overriding the method in a subclass and provide a conversion from string to the concrete type %s", string.getClass()), ex);
        }
        return null;
