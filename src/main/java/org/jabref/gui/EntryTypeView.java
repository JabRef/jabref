...
public class EntryTypeView extends BaseDialog<EntryType> {
    ...
    public EntryTypeView(
        ...
        ControlHelper.setAction(
            generateButton,
            this.getDialogPane(),
            event -> viewModel.runFetcherWorker()
        );
        ...
        btnGenerate
            ...
            .bind(
                EasyBind.map(
                    ...
                    searching ->
                        searching
                            ? Localization.lang("Searching...")
                            : Localization.lang("Generate")
                )
            );
        btnGenerate
            .disableProperty()
            .bind(
                viewModel
                    .idFieldValidationStatus()
                    .validProperty()
                    .not()
                    .or(viewModel.searchingProperty())
            );

        EasyBind.subscribe(
            viewModel.searchSuccesfulProperty(),
            isSuccessful -> {
                if (isSuccessful) {
