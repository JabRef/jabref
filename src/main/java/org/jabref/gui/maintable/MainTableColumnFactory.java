...
public class MainTableColumnFactory {
            ...
            case SPECIALFIELD:
                if (!column.getQualifier().isBlank()) {
                    ...
                    if (field instanceof SpecialField) {
                        ...
                        LOGGER.warn(
                            "Special field type '{}' is unknown. Using normal column type.",
                            column.getQualifier()
                        );
    ...
    private Node createGroupColorRegion(
        BibEntryTableViewModel entry,
        List<AbstractGroup> matchedGroups
    ) {
        List<Color> groupColors = matchedGroups
            .stream()
        ...
        if (!groupColors.isEmpty()) {
            ...
            String matchedGroupsString = matchedGroups
                .stream()
                .distinct()
     ...
     */
    private TableColumn<BibEntryTableViewModel, ?> createFieldColumn(
        MainTableColumnModel columnModel
    ) {
     ...
     */
    private TableColumn<BibEntryTableViewModel, String> createLibraryColumn(
        MainTableColumnModel columnModel
    ) {
