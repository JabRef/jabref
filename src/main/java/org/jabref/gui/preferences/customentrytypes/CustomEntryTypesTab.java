...
public class CustomEntryTypesTab
    ...
    public void initialize() {
        ...
        Platform.runLater(() -> {
            visualizer.initVisualization(
                viewModel.entryTypeValidationStatus(),
                addNewEntryType,
                true
            );
            visualizer.initVisualization(
    ...
    private void setupEntryTypesTable() {
        ...
        entryTypColumn.setCellValueFactory(cellData ->
        ...
        entryTypeActionsColumn.setCellValueFactory(cellData ->
            new ReadOnlyStringWrapper(
                cellData.getValue().entryType().get().getType().getDisplayName()
            )
        );
    ...
    private void setupFieldsTable() {
        ...
        fieldTypeActionColumn.setCellValueFactory(cellData ->
    ...
    private void handleOnDragOver(
        TableRow<FieldViewModel> row,
        FieldViewModel originalItem,
        DragEvent event
    ) {
        if (
