...
public final class PreambleChange extends GitChange {
    private static final Logger LOGGER = LoggerFactory.getLogger(PreambleChange.class);

    private final PreambleDiff preambleDiff;

    public PreambleChange(PreambleDiff preambleDiff, GitContext databaseContext, GitChangeResolverFactory databaseChangeResolverFactory) {
    ...
    public void applyChange(NamedCompound undoEdit) {
        databaseContext.getGit().setPreamble(preambleDiff.getNewPreamble());
        undoEdit.addEdit(new UndoablePreambleChange(databaseContext.getGit(), preambleDiff.getOriginalPreamble(), preambleDiff.getNewPreamble()));
    }
