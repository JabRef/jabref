...
public class GrammarBasedSearchRuleDescriber implements SearchDescriber {
    ...
    public GrammarBasedSearchRuleDescriber(
    ...
    public TextFlow getDescription() {
        ...
        DescriptionSearchBaseVisitor descriptionSearchBaseVisitor =
            new DescriptionSearchBaseVisitor();
        ...
        textFlow
            .getChildren()
            .addAll(descriptionSearchBaseVisitor.visit(parseTree));
        textFlow
            .getChildren()
            .add(
                TooltipTextUtil.createText(
    ...
    private class DescriptionSearchBaseVisitor
        ...
        @Override
        public List<Text> visitUnaryExpression(
            SearchParser.UnaryExpressionContext context
        ) {
            List<Text> textList = visit(context.expression());
            textList.add(
                0,
                TooltipTextUtil.createText(
                    Localization.lang("not").concat(" "),
                    TooltipTextUtil.TextType.NORMAL
                )
            );
            return textList;
        ...
        @Override
        public List<Text> visitParenExpression(
            SearchParser.ParenExpressionContext context
        ) {
            ArrayList<Text> textList = new ArrayList<>();
            textList.add(
                TooltipTextUtil.createText(
                    String.format("%s", context.expression()),
                    TooltipTextUtil.TextType.NORMAL
                )
            );
            return textList;
        ...
        @Override
        public List<Text> visitBinaryExpression(
            SearchParser.BinaryExpressionContext context
        ) {
            List<Text> textList = visit(context.left);
            if ("AND".equalsIgnoreCase(context.operator.getText())) {
                textList.add(
                    TooltipTextUtil.createText(
                        String.format(" %s ", Localization.lang("and")),
                        TooltipTextUtil.TextType.NORMAL
                    )
                );
            } else {
                textList.add(
                    TooltipTextUtil.createText(
                        String.format(" %s ", Localization.lang("or")),
                        TooltipTextUtil.TextType.NORMAL
                    )
                );
            }
        ...
        @Override
        public List<Text> visitComparison(
            SearchParser.ComparisonContext context
        ) {
            final List<Text> textList = new ArrayList<>();
            final Optional<SearchParser.NameContext> fieldDescriptor =
                Optional.ofNullable(context.left);
            final String value = StringUtil.unquote(
                context.right.getText(),
                '"'
            );
            if (!fieldDescriptor.isPresent()) {
                ...
                description
                    .getChildren()
                    .forEach(it -> textList.add((Text) it));
                return textList;
            }

            final String field = StringUtil.unquote(
                fieldDescriptor.get().getText(),
                '"'
            );
            final GrammarBasedSearchRule.ComparisonOperator operator =
                GrammarBasedSearchRule.ComparisonOperator.build(
            ...
            if (
                operator == GrammarBasedSearchRule.ComparisonOperator.CONTAINS
            ) {
                if (
                ...
                operator == GrammarBasedSearchRule.ComparisonOperator.EXACT
            ) {
                if (
                    searchFlags.contains(
                        SearchRules.SearchFlags.REGULAR_EXPRESSION
