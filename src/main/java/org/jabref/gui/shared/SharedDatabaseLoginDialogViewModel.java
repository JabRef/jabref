...
public class SharedDatabaseLoginDialogViewModel extends AbstractViewModel {
    ...
    private boolean openSharedDatabase(DBMSConnectionProperties connectionProperties) {
        ...
        try {
            ...
            result.filter(btn -> openHelp.equals(btn)).ifPresent(btn -> new HelpAction(HelpFile.SQL_DATABASE_MIGRATION, dialogService, preferences.getExternalApplicationsPreferences()).execute());
        }
        loading.set(false);
        return false;
    }
