...
public class RangeOverlapBetween {
    ...
    public static <V extends RangeHolder> List<RangeOverlap<V>> findFirst(
         ...
         * to LO.
         */
        List<OOTuple3<XText, XTextRangeCompare, V>> fewTuples = new ArrayList<>(
            fewHolders.size()
        );
        ...
        for (V bHolder : manyHolders) {
            ...
            for (OOTuple3<XText, XTextRangeCompare, V> tup : fewTuples) {
                ...
                int abEndToStart = UnoTextRange.compareStartsUnsafe(
                    cmp,
                    aRange.getEnd(),
                    bRangeStart
                );
                ...
                int baEndToStart = UnoTextRange.compareStartsUnsafe(
                    cmp,
                    bRangeEnd,
                    aRange.getStart()
                );
                ...
                result.add(
                    new RangeOverlap<>(kind, valuesForOverlappingRanges)
                );
                return result;
            }
        }
