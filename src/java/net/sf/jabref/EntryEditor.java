...
public class EntryEditor extends JPanel implements VetoableChangeListener {
  ...
  FieldPanel reqPanel = new FieldPanel(), optPanel = new FieldPanel(),
      genPanel = new FieldPanel(), absPanel = new FieldPanel();
  ...
  double optW = 0, reqW = 1, genW = 0; // Variables for total weight of
  ...
  public EntryEditor(JabRefFrame frame_, BasePanel panel_, BibtexEntry entry_,
    ...
    setupSourcePanel();
    tabbed.addTab(Globals.lang("Required fields"), new ImageIcon(
        GUIGlobals.showReqIconFile), reqPanel.getPane(), Globals
        .lang("Show required fields"));
    if ((entry.getOptionalFields() != null)
        && (entry.getOptionalFields().length >= 1)){
      tabbed.addTab(Globals.lang("Optional fields"), new ImageIcon(
          GUIGlobals.showOptIconFile), optPanel.getPane(), Globals
          .lang("Show optional fields"));
    }
    if ((entry.getGeneralFields() != null)
        && (entry.getGeneralFields().length >= 1)){
      tabbed.addTab(Globals.lang("General fields"), new ImageIcon(
          GUIGlobals.showGenIconFile), genPanel.getPane(), Globals
          .lang("Show general fields"));
    }
    tabbed.addTab("Abstract", new ImageIcon(GUIGlobals.showAbsIconFile),
        absPanel.getPane(), Globals.lang("Show abstract"));
    tabbed.addTab(Globals.lang("BibTeX source"), new ImageIcon(
        GUIGlobals.sourceIconFile), srcPanel, Globals
        .lang("Show/edit BibTeX source"));
    ...
    int prefHeight = (int) (Math.max(genW, Math.max(optW, reqW)) * GUIGlobals.FORM_HEIGHT[prefs
        .getInt("entryTypeFormHeightFactor")]);
    setSize(GUIGlobals.FORM_WIDTH[prefs.getInt("entryTypeFormWidth")],
        prefHeight);
      ...
      addMouseListener(new MouseAdapter() {
        ...
        public void mouseClicked(MouseEvent e) {
          ...
          if ((e.getButton() == MouseEvent.BUTTON3)
              || (ctrlClick && (e.getButton() == MouseEvent.BUTTON1) && e
                  .isControlDown())){
  ...
  private void setupFieldPanels(FieldPanel req, FieldPanel opt, FieldPanel gen,
    ...
    // required.
    String[] reqFields = entry.getRequiredFields(), optFields = entry
        .getOptionalFields(),
  ...
  private JComponent getExtra(String string, FieldEditor editor) {
    ...
    if ((s != null) && s.equals("external")){
      ...
      FieldContentSelector ws = new FieldContentSelector(this, editor,
          panel.metaData);
      ...
      but.addActionListener(new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
          ...
          if (chosenFile != null){
            ...
            prefs
                .put(fieldName + Globals.FILETYPE_PREFS_EXT, newFile.getPath());
            updateField(ed);
          }
        }
      });
      ...
      JButton but = new JButton(Globals.lang("Browse")), download = new JButton(
        ...
        public void actionPerformed(ActionEvent e) {
          ...
          if (chosenFile != null){
            ...
            if ((pdfDir != null) && position.startsWith(pdfDir)){
              ...
              String relPath = position.substring(pdfDir.length(), position
                  .length())
                  + File.separator + newFile.getName();
              // Remove leading path separator
              if (relPath.startsWith(File.separator)){
                relPath = relPath.substring(File.separator.length(), relPath
                    .length());
      ...
      download.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          String res = JOptionPane.showInputDialog((Component) ed, Globals
              .lang("Enter URL to download"));
          if (res != null){
            URL url;
            try{
              url = new URL(res);
              File file = new File(new File(prefs.get("pdfDirectory")), entry
      ...
      ((JComponent) editor).setDropTarget(new DropTarget((Component) editor,
          DnDConstants.ACTION_NONE, new SimpleUrlDragDrop(editor,
              storeFieldAction)));
      return null;
    }else{
      return null;
  ...
  class TabListener implements ChangeListener {
    ...
    public void stateChanged(ChangeEvent e) {
      if (((JTabbedPane) e.getSource()).getSelectedIndex() != sourceIndex){
        FieldPanel fp = (FieldPanel) (((JTabbedPane) e.getSource())
  ...
  class DeleteAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      panel.refreshTable();
      panel.undoManager.addEdit(new UndoableRemoveEntry(panel.database, entry,
          panel));
  ...
  class CloseAction extends AbstractAction {
    ...
    public CloseAction() {
      super(Globals.lang("Close window"), new ImageIcon(
  ...
  class CopyKeyAction extends AbstractAction {
    ...
    public CopyKeyAction() {
      super("Copy BibTeX key to clipboard", new ImageIcon(
          GUIGlobals.copyKeyIconFile));
      ...
      if (e.getSource() instanceof FieldTextArea){
        ...
        if (set){
          try{
            ...
            if (toSet != null){
              (new LatexFieldFormatter()).format(toSet, GUIGlobals
                  .isStandardField(fe.getFieldName()));
            }
            ...
            // undoManager.
            panel.undoManager.addEdit(new UndoableFieldChange(entry, fe
                .getFieldName(), oldValue, toSet));
            updateSource();
            panel.refreshTable();
            panel.markBaseChanged();
        ...
        if (((oldValue == null) && (newValue == null))
            || ((oldValue != null) && (newValue != null) && oldValue
                .equals(newValue))){ return; // No change.
        }
        boolean isDuplicate = panel.database.setCiteKeyForEntry(entry.getId(),
            newValue);
        if (newValue != null){
          if (isDuplicate){
            ...
            if (prefs.getBoolean("dialogWarningForDuplicateKey")){
              ...
              JOptionPane.showMessageDialog(frame, jcb,
                  Globals.lang("Warning"), JOptionPane.WARNING_MESSAGE);
              if (jcb.isSelected()){
        ...
        panel.undoManager.addEdit(new UndoableKeyChange(panel.database, entry
  ...
  class GenerateKeyAction extends AbstractAction {
    ...
    public GenerateKeyAction(JabRefFrame parentFrame) {
      super(Globals.lang("Generate BibTeX key"), new ImageIcon(
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      try{
        ...
        panel.undoManager.addEdit(new UndoableKeyChange(panel.database, entry
            .getId(), (String) oldValue, (String) entry
            .getField(GUIGlobals.KEY_FIELD)));
  ...
  public boolean storeSource(boolean showError) {
    ...
    BibtexParser bp = new BibtexParser(new java.io.StringReader(source
        .getText()));
    try{
      BibtexDatabase db = bp.parse().getDatabase();
      if (db.getEntryCount() > 1){ throw new Exception(
      ...
      for (int i = 0; i < fields.length; i++){
        if (entry.getField(fields[i].toString()) != nu.getField(fields[i]
            .toString())){
          String toSet = (String) nu.getField(fields[i].toString());

          // Test if the field is legally set.
          (new LatexFieldFormatter()).format(toSet, GUIGlobals
              .isStandardField(fields[i].toString()));
      ...
      if (showError){
        ...
        int answer = JOptionPane.showOptionDialog(frame, "Error: "
            + ex.getMessage(), Globals.lang("Problem with parsing entry"),
  ...
  public boolean setField(String fieldName, String newFieldData) {
    ...
    try{
      ...
      if (setFieldInPanel(reqPanel, fieldName, newFieldData)){ return true; }
      if (setFieldInPanel(optPanel, fieldName, newFieldData)){ return true; }
      if (setFieldInPanel(genPanel, fieldName, newFieldData)){ return true; }
