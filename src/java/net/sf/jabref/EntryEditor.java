...
//    generateKeyAction = new GenerateKeyAction(baseFrame,entry);
    ...
    int prefHeight = ( int ) ( Math.max( genW, Math.max( optW, reqW ) ) *
                               GUIGlobals.FORM_HEIGHT[prefs.getInt(
...
//        genFields = new String[] {"crossref", "url", "abstract", "comment"}; // May change...
    ...
    FieldTextArea ta1 = null, ta2 = null, ta3 = null, ta4 = null, firstR = null,
        firstO = null ;
        ...
        public void actionPerformed( ActionEvent e )
        {
          ...
          if ( chosenFile != null )
          {
            ...
            if ( ( pdfDir != null ) && position.startsWith( pdfDir ) )
            {
              ...
              String relPath = position.substring( pdfDir.length(),
                  position.length() ) + File.separator + newFile.getName() ;
        ...
        public void actionPerformed( ActionEvent e )
        {
          ...
          if ( ( o == null ) || ( prefs.get( "pdfDirectory" ) == null ) )
          {
            frame.output( Globals.lang(
                "You must set both bibtex key and PDF directory" ) + "." ) ;
            return ;
          }
          ...
          ( new Thread()
          {
            ...
            {
              Object o = entry.getField( Globals.KEY_FIELD ) ;
              String found = Util.findPdf( ( String ) o,
                                           prefs.get( "pdfDirectory" ) ) ;
              if ( found != null )
              {
                ed.setText( found ) ;
                storeFieldAction.actionPerformed( new ActionEvent( ed, 0, "" ) ) ;
  ...
  private void setupJTextComponent( JTextComponent ta )
  {
    ...
    try
    {
      HashSet keys = new HashSet( ta.getFocusTraversalKeys(
          KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS ) ) ;
      keys.clear() ;
      keys.add( AWTKeyStroke.getAWTKeyStroke( "pressed TAB" ) ) ;
      ta.setFocusTraversalKeys( KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS,
                                keys ) ;
      keys = new HashSet( ta.getFocusTraversalKeys( KeyboardFocusManager.
  ...
  public void setEnabled( boolean enabled )
  {
    ...
    FieldPanel[] panels = new FieldPanel[]
        {
        reqPanel, optPanel, genPanel} ;
...
//            selectedEntry = newEntry ;
      ...
      try
      {
        ...
        panel.undoManager.addEdit( new UndoableKeyChange
                                   ( panel.database, entry.getId(),
                                     ( String ) oldValue,
                                     ( String ) entry.getField( GUIGlobals.
            KEY_FIELD ) ) ) ;
  ...
  private boolean setFieldInPanel( FieldPanel pan, String fieldName,
                                   String newFieldData ) throws
      ClassCastException
  {
    Vector fields = pan.getFields() ;
    for ( int i = 0 ; i < fields.size() ; i++ )
    {
      if ( ( ( FieldEditor ) fields.elementAt( i ) ).getFieldName().equals(
          fieldName ) )
      {
  ...
  public void updateAllFields()
  {
    FieldPanel[] panels = new FieldPanel[]
        {
        reqPanel, optPanel, genPanel, absPanel} ;
    for ( int i = 0 ; i < panels.length ; i++ )
    {
      Vector fields = panels[i].getFields() ;
      for ( int j = 0 ; j < fields.size() ; j++ )
