...
public class FileActions {
    ...
    public static void saveDatabase(BibtexDatabase database, MetaData metaData,
				    ...
				    boolean checkSearch, boolean checkGroup) 
	...
	try {
		...
		if (back.exists()) back.renameTo(temp);
		...
		boolean write = true;
		if (checkSearch &&
		    !nonZeroField(be, Globals.SEARCH))
		    write = false;
		if (checkGroup &&
		    !nonZeroField(be, Globals.GROUPSEARCH))
		    write = false;
		...
		path = file.getParent();
	    File temp = new File(path, name+GUIGlobals.tempExt),
		back = new File(path, name+GUIGlobals.backupExt);
	    if (file.exists()) file.delete();
	    if (temp.exists()) temp.renameTo(file);		    
	    else 
		back.renameTo(file);
