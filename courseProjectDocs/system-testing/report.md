...
| **TC-04: Verify Optional Field Volume** | JabRef project builds successfully | 1. Create `BibEntry` <br> 2. Set `VOLUME` field to `VOL. 1` <br> 3. Create DataBase <br> 4. Append BibEntry to BibDatabase <br> 5. Return set Optional from BibEntry as expected <br> 6. Have Actual as `getField()` value <brb> 7. Compare actual to expected                                                   | BibEntry stores `VOL.1` for VOLUME StandardField <br> expected value is `VOL.`                                                                                                                                                                                                                                                                             | 
