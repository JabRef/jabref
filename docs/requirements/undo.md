...
## Undo journal must identify saved position by state identity
...
## Library must maintain isolated undo history
...
## Undo journal must apply and record undoable changes atomically
...
## Field editors must route undo shortcuts to library undo history
...
## Undo journal must decline undo while command holds pending writes
...
## Undo journal must refuse stale change when library state has advanced
...
## Group operations must record on undo stack as single step
...
## Library modified marker must derive state from undo journal position
...
## Library settings updates must record on undo stack as single step
...
## Text typing must record on undo stack word by word
...
## Entry insertion must record on undo stack as single step
