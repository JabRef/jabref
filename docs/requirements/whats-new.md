...
Clicking the button fetches again, lists what that fetch found and only then, when JabRef was started by the `just run-loop` recipe, offers *Restart to update*, which quits JabRef the ordinary way and leaves a marker for the recipe to pull, rebuild and start it again.
