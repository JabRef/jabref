...
Users can define custom entry editor tabs in the preferences ("Entry editor" → "Editor tabs"): a "Tabs" column lists all tabs (built-in tabs with a visibility checkbox, custom tabs with a delete action) and supports adding custom tabs and reordering all tabs via drag and drop; a "Fields" column edits the selected custom tab's ordered field list, also reorderable via drag and drop. A field entry is either a plain field name (always shown on the tab, even while unset) or a regular expression (e.g. `comment-.*`), which shows every set field of the entry whose name matches. A field listed on more than one tab is marked with a warning sign. A field shown on a custom tab is omitted from the Main tab's remaining-fields list; fields the entry type defines as required or optional (and the citation key) always stay in the Main tab. Custom tabs configured in JabRef versions before the "Main" tab rework are picked up again without migration.
...
The "Editor tabs" preferences offer a button "Add classic 5.x tabs" which adds the custom tabs "General" (the JabRef 5 default field set: doi, icore, citationcount, crossref, keywords, eprint, eprinttype, url, file, groups, owner, timestamp, and all special fields), "Abstract" (abstract), and "Comments" (comment and the user-specific comment-* fields) directly after the "Main" tab. A classic tab is not added when a tab of that name exists or a custom tab already has exactly its fields, so the button is idempotent. The classic tabs' names are stored as localization keys (`%General`, `%Abstract`) and translated when displayed, so they follow a language change, whereas user-typed tab names are stored and shown verbatim. When upgrading from a version before the "Main" tab rework, stored tabs that are exactly one of the former default "General"/"Abstract" tabs (localized name and shipped field set) are not carried over, since the "Main" tab shows all their fields; any other stored tab is kept.
