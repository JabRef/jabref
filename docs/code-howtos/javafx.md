...
Style with `styleClass`, never with a node `id`: an id is how a walkthrough step finds the control it highlights, so styling by id competes with that use. The only ids the stylesheets still select on belong to JavaFX's own custom-color dialog, whose nodes we do not build.
