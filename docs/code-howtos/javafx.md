...
* Write ids in kebab case (`entry-editor`), the same way style classes are written. An id JavaFX derives from an `fx:id` is the exception, since that has to be a Java identifier.
