...
rely
on
basic [JUnit](https://junit.org/junit5/docs/current/user-guide/)
unit
tests
to
increase
...
see
also
the [Stackoverflow discussion about naming](http://stackoverflow.com/questions/155436/unit-test-naming-best-practices)).
*
_Test
only
one
...
etc.).
See [JUnit AntiPattern](https://exubero.com/junit/anti-patterns/#Multiple_Assertions)
for
background.
*
Do
...
feature
of [junit-jupiter](https://junit.org/junit5/docs/current/user-guide/)
to
the
test
method.
...
Read
more
at <https://mikemybytes.com/2021/10/19/parameterize-like-a-pro-with-junit-5-csvsource/>.
...
are
run.
See <https://www.geeksforgeeks.org/junit-5-tempdir/>
for
more
details.
...
marked
with
`@org.jabref.testutils.category.DatabaseTest`.

### Database tests

JabRef
can [use an external PostgreSQL database to store bibliographic data](https://docs.jabref.org/collaborative-work/sqldatabase).
The
tests
require
such
...
is
available
at [JabRef's fetcher documentation](fetchers.md).
...
marked
by
`@org.jabref.testutils.category.FetcherTest`.
Some
of
them
are
...
Tool
window (
`JabRef > Tasks > other > fetcherTest`).
