...
keywords,
in
`{prefix}_MARKUP_BEFORE`,
`{prefix}_MARKUP_AFTER`
pairs
to
allow
bracketing
...
ties.
*
`RangeSort.partitionAndSortRanges` :
since
`XTextRangeCompare`
can
only
...
databases:
*
`CitationGroups.lookupCitations`
collects
the
cited
keys,
...
`CitationGroups.imposeLocalOrder`
fills
`localOrder`
in
each
...
logic,
in
`OOProcessAuthorYearMarkers.createUniqueLetters()`
...
self-contained
entries
`List<CitationMarkerNumericEntry>`,
`List<CitationMarkerEntry>`.
*
We
have
distinct
methods
for
`getNormalizedCitationMarker(CitationMarkerNormEntry)`
and
