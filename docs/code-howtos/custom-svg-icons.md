...
Use the [IcoMoon](https://icomoon.io) tool for packing the icons. Create a new set and import _all_ icons from the existing json file. The json file is in the jabref-admin repo
Rearrange them so that they have the same order as in `org.jabref.gui.JabRefMaterialDesignIcon`. This will avoid that you have to change the code points for the existing glyphs. In the settings for your icon set, set the _Grid_ to 24. This is important to get the correct spacing. The name of the font is `JabRefMaterialDesign`. When your icon-set is ready, select all of them and download the font-package.
