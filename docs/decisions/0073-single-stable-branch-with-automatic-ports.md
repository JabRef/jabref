...
* Before the merge, the port is simulated as a required check ("Would merge into the other branch", which also reports on the merge queue's commit), so a conflict is visible on the pull request itself. After the merge, CI cherry-picks the squash commit into a port pull request using [korthout/backport-action](https://github.com/korthout/backport-action), which auto-merges when CI passes. If the cherry-pick conflicts, the port pull request is opened as a draft whose commit contains the conflicting files with Git's conflict markers left in, and the workflow run fails: a maintainer resolves the markers in that pull request.
