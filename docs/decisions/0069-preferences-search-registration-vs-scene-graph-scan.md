...
Declaring them has one structural gap. The builder's escape hatch `custom(Node)` takes a node the caller assembled, so the builder does not know what text that node contains and can declare nothing for it. Text inside a hand-built row or a reused FXML panel is then absent from the search — with no compile error, no failing test, and a dialog that looks correct. How should the dialog determine what its search can find?

## Decision Drivers

* A gap in the search is invisible. The failure mode is a user who cannot find a setting.
* `custom(...)` is a permanent part of the builder API, so any mechanism relying on callers remembering to opt in decays as tabs are added.
...
* The preference tabs are builder-based; the scene graph reached through `custom(...)` is mostly hand-built rows and reusable FXML panels.

## Considered Options

* Registration only — the builder registers what it places, `custom(...)` contributes nothing
* Scene-graph scan only — the handler walks each tab's content, the builder registers nothing
* Registration plus an explicit opt-in per `custom(...)` call
* Registration plus an automatic scan of the subtree passed to `custom(...)`

## Decision Outcome

Chosen option: "Registration plus an automatic scan of the subtree passed to `custom(...)`", because it is the only option that leaves no gap behind a `custom(...)` call without requiring anything of the caller, while keeping the precise text-to-node captioning and the narrowed match set that registration provides.
...
* `ElementBase#searchable(String...)` is the explicit escape hatch for text held by no `Labeled`, and for a custom node populated only after `custom(...)` runs.
...
* Good, because a tab is searchable by construction — via registration for builder-placed content, via the scan for whatever it hands to `custom(...)`.
* Good, because no visible text is spelled twice: the scan reads back the string the layout already set.
* Good, because traversal is confined to the escape hatch, so the ordinary path stays declarative and the handler stays free of scene-graph knowledge.
* Bad, because the scan requires the custom node to be fully populated when `custom(...)` is called; a lazily populated node is silently missed and has to use `searchable(String...)`.
...
* Bad, because everything behind `custom(...)` is unfindable, with no API to change that.
* Bad, because each further `custom(...)` widens the gap, invisibly.
...
### Registration plus explicit opt-in per `custom(...)`
